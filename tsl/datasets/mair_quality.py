...
class MAirQualitySplitter(Splitter):

    def __init__(self, val_len: int = None,
                 test_months: Sequence = (3, 6, 9, 12)):
        super(MAirQualitySplitter, self).__init__()
...
class MAirQuality(PandasDataset, MissingValuesMixin):
    ...
    def load_raw(self):
        ...
        if self.small:
            ...
            eval_mask = None
        ''' 
        {'co': <HDF5 group "/co" (4 members)>,
        'distances': <HDF5 group "/distances" (4 members)>,
        'mask': <HDF5 group "/mask" (4 members)>,
        'no2': <HDF5 group "/no2" (4 members)>,
        'o3': <HDF5 group "/o3" (4 members)>,
        'pm10': <HDF5 group "/pm10" (4 members)>,
        'pm25': <HDF5 group "/pm25" (4 members)>,
        'so2': <HDF5 group "/so2" (4 members)>,
        'stations': <HDF5 group "/stations" (6 members)>}
        '''
        df1 = pd.read_hdf(path, 'pm25')
        df2 = pd.read_hdf(path, 'o3')
        df3 = pd.read_hdf(path, 'so2')
        df = pd.concat([df1,df2, df3], axis=1,keys=['pm25', 'o3', 's03'])\
    ...
    def get_splitter(self, method: Optional[str] = None, **kwargs):
        if method == 'air_quality':
            ...
            return MAirQualitySplitter(test_months=self.test_months,
                                      val_len=val_len)
